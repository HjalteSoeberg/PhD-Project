...
When estimating cause-specific risks in practice, the standard approach is to estimate them separately. However, this is suboptimal when the goal is to report both risks simultaneously, since the risks are interdependent. It is therefore necessary to estimate the cause-specific risks jointly, under appropriate constraints derived from the illness–death model. This issue has been examined previously, and a potential solution was proposed by (\citeprocitem{4}{Li \& Yang, 2016}). However, that solution was developed only for the Cox proportional hazards model and has not been extended to other modeling frameworks.
