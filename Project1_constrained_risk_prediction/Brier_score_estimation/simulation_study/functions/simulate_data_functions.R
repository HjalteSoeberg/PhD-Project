...
sim_cox_weibull <- function(n){
    ...
    distribution(m,~s) <- normal.lvm(mean = 0, sd = 0.8)
    distribution(m,~death) <- coxWeibull.lvm(scale=0.2, shape = 1)
    distribution(m,~y) <- a <- coxWeibull.lvm(scale=0.1,shape= 0.8)
    eventTime(m) <- times ~ min(death = 2,y=1)
    d <- lava::sim(m,n)
    d <- setDT(d)
    setnames(d,c("status","s"),c("event","X"))
...
sim_cox_weibull_cens <- function(n, C = 0.032){
    m <- lvm()
    regression(m) <- y ~ s
    regression(m, death~s) <- -0.1
    distribution(m,~s) <- normal.lvm(mean = 0, sd = 0.8)
...
sim_dpnext <- function(n){
    ...
    d <- lava::sim(m,n)
    d <- setDT(d)
    setnames(d,c("time"),c("times"))
    d[,y := NULL]
    d[,death := NULL]
    d[]
}

simulateStenoT1 <- function(n,
    ...
    d[,':='(time.event.1 = NULL,time.event.2 = NULL,time.event.0 = NULL,value_Motion = NULL,value_Smoking = NULL,value_Albuminuria = NULL)]
    setnames(d,"time","times")
    return(d[])
}
