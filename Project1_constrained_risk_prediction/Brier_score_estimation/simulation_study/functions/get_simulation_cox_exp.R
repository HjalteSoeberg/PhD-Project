get_simulation_cox_exp <- function(n,N,tau,true_par){
    if (FALSE){
        tar_load_globals()
        n = 300
        N = 10
    ...
    for (k in 1:N){
        tryCatch({
            d <- sim_cox_exp(n, true_par = as.numeric(true_par))
            ## d <- sim_dpnext(n)
            ## d <- sim_cox_weibull(n)
    ...
    df <- data.table(Model = c("logistic", "CSC", "Likelihood","Brier","Oracle","Brier_beta"),
                     Mean = c(100*mean(logi), 100*mean(cscc), 100*mean(LL), 100*mean(BS), 100*mean(true), 100*mean(BS_beta)))
    setkey(df,Mean)
    df[]
}
...
######################################################################
### get_simulation_cox_exp.R ends here
