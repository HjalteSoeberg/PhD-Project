...
list(
    ...
    tar_target(
        ...
        command = get_simulation_cox_exp_cens(n = 500,N = 40,C = 0.3,tau = 1, true_par <- c(beta1 = 1.1, beta2 = -0.6, lambda01 = 0.8, lambda02 = 0.2))
    ),
    tar_target(
        name = simulation_non_prop_cens,
        command = get_simulation_non_prop_cens(n = 300,N = 40,tau = 4, C = 0.3)
    )
