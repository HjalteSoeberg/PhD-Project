...
- check if problem happens with categorical covariates (probably not)  
...
- Problem may not happen if we use randomforest for one/both causes because randomforest does not extrapolate, it is a nearest neighbor method  (see Lin, Y., & Jeon, Y. (2006). Random Forests and Adaptive Nearest Neighbors. Journal of the American Statistical Association, 101(474), 578–590.).
