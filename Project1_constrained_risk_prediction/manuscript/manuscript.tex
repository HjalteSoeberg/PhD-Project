...
\label{sec:orged6ddcd}

\subsection*{Likelihood function}
\label{sec:org0718415}
...
\subsubsection*{Using Cox-Weibull}
\label{sec:org5e07efb}
...
\section*{Non-convexity of BSS}
\label{sec:orga42c5b6}
The BSS can be shown to be non-convex\ldots{}
...
\section*{How to handle censoring}
\label{sec:org0ca466e}
Implement how to handle censoring in the simulation setup to test if it works and then write about it here. Its just the setup from the joint survival superlearner. Remember to use the Brier-score-sum for all causes+censoring when fitting, but then to show performance only use the Brier-score-sum for the two causes.

\section*{Simulation study}
\label{sec:org5db222d}

We want to test our method in three different setups: first where the model is correctly specified, second where the model is not correctly specified but still proportional hazards and one where the proportional hazards assumption doesnt hold. We test our estimation method against the maximum likelihood estimator.

\subsection*{Correctly specified model}
\label{sec:org22b71fd}
See file "\textasciitilde{}/PhD-Project/Project1\textsubscript{constrained}\textsubscript{risk}\textsubscript{prediction}/Brier\textsubscript{score}\textsubscript{estimation}/does\textsubscript{BS}\textsubscript{find}\textsubscript{datagenerating}\textsubscript{parameters.R}" for example.

Brier-score estimation does indeed find the data-generating parameters, but can fail if the starting parameters for the newton-raphson algortihm are chosen poorly.

\subsection*{Choosing optimizer}
\label{sec:org77551ab}
\end{document}
