...
** The easy peasy constraint on S (prodlim) 
...
* Parametric Cox competing risks with binary x (problem doesnt arise)
...
** Seeing if and how F_1 + F_2 >1 (REWRITE AS THE CONSTRAINT CANT BE USED)
...
* Parametric Cox competing risks with constant or stepwise constant baseline hazard (poisson regression) 

** Constant baseline hazard (exponential distribution)
Consider the sum
#+begin_export latex
\begin{align*}
F_1(\tau|x)+F_2(\tau|x)
...
** Stepwise constant baseline hazard (poisson regression)
...
* New way - Brier-score with constraint (should write this into other sections)
